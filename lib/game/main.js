					...
					if(unit.turn == this.turn && unit.team === this.team){
						...
						if(unit.target > -1){
							...
							if(unit.team === 2 && this.map.pointZero.x + (unit.target * 16) < unit.pos.x){
								...
								// unit gets to the enemy line - convert to flag - fix: and there is no unit!			
	...
	draw: function() {		
		...
		if(this.STATE == 'game' || this.STATE == 'move' || this.STATE == 'game_menu' || this.STATE == 'game_over'){			
			...
			if(this.STATE == 'game'){
				...
				this.fontWhite.draw('BUY UNITS:', newX+14, newY-48, ig.Font.ALIGN.CENTER );

				this.cpuSheet.drawTile(newX, newY-44,0,16,16);
				this.fontWhite.draw(this.teams[this.team-1].bitcoins.toFixed(1), newX+18, newY-38, ig.Font.ALIGN.LEFT );
				...
				if(this.settings.prices.soldier <= this.teams[this.team-1].bitcoins){
					this.fontWhite.draw(this.settings.prices.soldier, newX+24, newY+2-24, ig.Font.ALIGN.CENTER );
				}else{
					this.fontColor.draw(this.settings.prices.soldier, newX+24, newY+2-24, ig.Font.ALIGN.CENTER );
				}

				if(this.settings.prices.tank <= this.teams[this.team-1].bitcoins){	
					this.fontWhite.draw(this.settings.prices.tank, newX+24, newY+2, ig.Font.ALIGN.CENTER );	
				}else{
					this.fontColor.draw(this.settings.prices.tank, newX+24, newY+2, ig.Font.ALIGN.CENTER );	
				}

				if(this.settings.prices.helicopter <= this.teams[this.team-1].bitcoins){	
					this.fontWhite.draw(this.settings.prices.helicopter, newX+24, newY+2+24, ig.Font.ALIGN.CENTER );
				}else{
					this.fontColor.draw(this.settings.prices.helicopter, newX+24, newY+2+24, ig.Font.ALIGN.CENTER );
				}
