...
MyGame = ig.Game.extend({
		...
		for (var x = 0; x < this.map.blueprint.width; x++) {
			for (var y = 0; y < this.map.blueprint.height; y++) {				
				...
				if(this.map.terrain[x][y].type === 5){
					name = 'base_'+Math.random().toString();
					...
					if(unit.turn == this.turn && unit.team === this.team){
						...
						if(!unit.flag){
							...
							if(!unit.target){
								...
								// next field is empty
								if(!this.map.units[targetX][y] && (this.map.terrain[targetX][y].type < unit.terrain || this.map.terrain[targetX][y].type > 2)){
									// if there is a base? 
									if(this.map.terrain[targetX][y].base){
										// and its not my team?
	...
	draw: function() {		
		...
		if(this.STATE == 'game'){
			...
			this.fontWhite.draw('DONE', this.GUI.centerBottom.x, this.GUI.centerBottom.y-9, ig.Font.ALIGN.CENTER );		
		}
		...
		if(this.STATE == 'game' || this.STATE == 'move' || this.STATE == 'terrain'){			
			...
			this.fontRed.draw(this.teams[1].score+' - RED SCORE', this.GUI.rightTop.x, this.GUI.rightTop.y, ig.Font.ALIGN.RIGHT );		
