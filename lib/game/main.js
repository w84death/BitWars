...
MyGame = ig.Game.extend({
	...
	stateChange: function(state){
		if ( this.STATE == 'intro' && state == 'menu' ){
			this.menu.cursor.push(this.GUI.centerMiddle.y-24);
			this.menu.cursor.push(this.GUI.centerMiddle.y-12);
			this.menu.cursor.push(this.GUI.centerMiddle.y);
			this.STATE = 'menu';	
		}

		if ( this.STATE == 'menu' && state == 'newGame' ){
			this.STATE = 'game';			
			if(this.menu.position === 0){
	...
	moveCursor: function(params){
		if(params.menu){
			if(params.up){
				...
				if(this.menu.position < 0){
					this.menu.position = this.menu.cursor.length-1;
				}		
			}
			if(params.down){
				this.menu.position += 1;
				if(this.menu.position > this.menu.cursor.length-1){
	...
	update: function() {
		...
		if(this.STATE == 'menu'){
			if(ig.input.released('up')){
				this.moveCursor({up:true, menu:true});
			}
			if(ig.input.released('down')){
				this.moveCursor({down:true, menu:true});
			}
			if(ig.input.released('action') || ig.input.state('touch')){				
	...
	draw: function() {		
		...
		if(this.STATE == 'intro'){
			...
			if( this.timer.delta().toFixed(0) % 2 != 0){
				this.fontWhite.draw('PRESS START [enter]', this.GUI.centerBottom.x, this.GUI.centerBottom.y-48, ig.Font.ALIGN.CENTER );
			}
		...
		if(this.STATE == 'menu'){
			...
			if(this.menu.position == 0){
				this.fontColor.draw('HUMAN VS AI', this.GUI.centerMiddle.x, this.menu.cursor[0], ig.Font.ALIGN.CENTER );
			}else{
				this.fontWhite.draw('HUMAN VS AI', this.GUI.centerMiddle.x, this.menu.cursor[0], ig.Font.ALIGN.CENTER );
			}
			if(this.menu.position == 1){
				this.fontColor.draw('HUMAN VS HUMAN', this.GUI.centerMiddle.x, this.menu.cursor[1], ig.Font.ALIGN.CENTER );
			}else{
				this.fontWhite.draw('HUMAN VS HUMAN', this.GUI.centerMiddle.x, this.menu.cursor[1], ig.Font.ALIGN.CENTER );
			}
			if(this.menu.position == 2){
				this.fontColor.draw('AI VS AI', this.GUI.centerMiddle.x, this.menu.cursor[2], ig.Font.ALIGN.CENTER );
			}else{
				this.fontWhite.draw('AI VS AI', this.GUI.centerMiddle.x, this.menu.cursor[2], ig.Font.ALIGN.CENTER );
			}
			this.heroesImage.drawTile(this.GUI.centerTop.x-72, this.menu.cursor[this.menu.position]-12,1,30,28);
			this.heroesImage.drawTile(this.GUI.centerTop.x+42, this.menu.cursor[this.menu.position]-12,0,30,28);			
		...
		if(this.STATE == 'game' || this.STATE == 'move' || this.STATE == 'terrain' || this.STATE == 'game_over'){			
			this.renderMapFrame();
			...
			if(this.STATE == 'game'){
				...
				this.fontWhite.draw('BUY UNITS:\nBTC: '+this.teams[this.team-1].bitcoins.toFixed(1), newX+14, newY-48, ig.Font.ALIGN.CENTER );
