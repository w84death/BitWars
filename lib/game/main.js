...
MyGame = ig.Game.extend({
	...
	// ------------------------------------------------------------------------
						...
						version: 		'FINAL',
						messageUnderLogo: 'Celebrating 10.000 play sessions !',
	...
	p1xImage: 		new ig.Image( 'media/gui/p1x.png' ),
	...
	tutorialBasesImage: 	new ig.Image( 'media/tutorial/bases.png' ),
	tutorialCpusImage: 	new ig.Image( 'media/tutorial/cpus.png' ),
	tutorialUnitsImage: 	new ig.Image( 'media/tutorial/units.png' ),
	tutorialGoalImage: 	new ig.Image( 'media/tutorial/goal.png' ),
	tutorialMovesImage: 	new ig.Image( 'media/tutorial/moves.png' ),
	tutorialTilesImage: 	new ig.Image( 'media/tutorial/tiles.png' ),
	...
	init: function() {
		...
		this.GUI.centerMiddle = {x:ig.system.width*0.5,y:ig.system.height*0.5+5};
		...
		this.menu['game_menu'].cursor.push(this.GUI.centerMiddle.y+12);
		this.menu['game_menu'].cursor.push(this.GUI.centerMiddle.y+24);
		this.menu['game_menu'].cursor.push(this.GUI.centerMiddle.y+48);

		this.menu['game_over'].cursor.push(this.GUI.centerMiddle.y);
		this.menu['game_over'].cursor.push(this.GUI.centerMiddle.y+12);
		this.menu['game_over'].cursor.push(this.GUI.centerMiddle.y+24);
        ...
        this.fontOUYA.letterSpacing = -1;
        this.fontOUYA.lineSpacing = -2;
		...
		ig.game.spawnEntity( EntityMap_frame, 0, 0, {width:ig.system.width, height:ig.system.height} );
	...
	stateChange: function(state){
		...
		if ( this.STATE == 'tutorial' && state == 'next_page' ){
			if(this.menu['tutorial'].page < this.menu['tutorial'].pages){
				this.menu['tutorial'].page += 1;
			}else{
				this.menu['tutorial'].page = 1;
				this.menu['menu'].position = 0;
				this.STATE = 'menu';
			}
		}
		...
		if((this.STATE == 'game_menu' || this.STATE == 'game_over') && state == 'new_map'){
			var params = {};
			if(this.menu['map'].position === 0){
				params.width = 9;
				params.height = 4;
			...
			for (var y = 0; y < this.map.blueprint.height; y++) {
				if(this.map.terrain[x][y].base){
					...
					if(base.team === this.team){
						...
						ig.game.spawnEntity( EntityParticle_cpu, base.pos.x, base.pos.y, {team:base.team} );
					}
				}
			}
		}
	...
	nextTurn: function(){
		...
		if(this.teams[this.team-1].ai){
			this.teams[this.team-1].moves += 4;
			this.ai.unlock();
		}
		this.teams[this.team-1].timer.unpause();
	},
	...
	shakeMap: function(){
		...
		if(this.shake.delta() < 0.1){
			if(Math.random()*10 > 5){
				newX = 8 - ((Math.random()*16)<<0);
				newY = 8 - ((Math.random()*16)<<0);
				this.screen.x += newX;
				this.screen.y += newY;
				this.map.pointZero.x += -newX;
				this.map.pointZero.y += -newY;
			}else{
				this.screen.x = 0;
				this.screen.y = 0;
				this.map.pointZero.x = this.map.pointZero.def.x;
				this.map.pointZero.y = this.map.pointZero.def.y;
			}
		}else
		if(this.shake.delta() > 0.1 && this.shake.delta() < 0.2){
			this.screen.x = 0;
			this.screen.y = 0;
			this.map.pointZero.x = this.map.pointZero.def.x;
			this.map.pointZero.y = this.map.pointZero.def.y;
		...
		if(this.STATE == 'menu' || this.STATE == 'map' || this.STATE == 'game_menu' || this.STATE == 'game_over'){
			...
			if(ig.input.released('action') || ig.input.released('next_turn')){
				...
				if(this.STATE == 'game_menu'){
					...
					if(this.menu['game_menu'].position === 3){
						this.stateChange('new_map');
					}
					if(this.menu['game_menu'].position === 4){
						this.sound = !this.sound;
						if(!this.sound){
							ig.music.stop();
						}
				...
				if(this.STATE == 'game_over'){
					...
					if(this.menu['game_over'].position === 2){
						this.stateChange('menu');
					}
				}
				return;
	...
	draw: function() {
		...
		if(this.STATE == 'intro' || this.STATE == 'menu' || this.STATE == 'map' || this.STATE == 'game_over' || this.STATE == 'game_menu'){
			this.p1xImage.draw(8,ig.system.height-38);
		}

		if(this.STATE == 'intro'){
			this.logoImage.draw(this.GUI.centerTop.x-24, this.GUI.centerTop.y+6);
			this.fontWhite.draw(this.messageUnderLogo, this.GUI.centerTop.x, this.GUI.centerTop.y+32, ig.Font.ALIGN.CENTER );

			this.heroesImage.drawTile(this.GUI.centerMiddle.x-72, this.GUI.centerMiddle.y-12,1,30,28);
			this.heroesImage.drawTile(this.GUI.centerMiddle.x+42, this.GUI.centerMiddle.y-12,0,30,28);

			if( (this.timer.delta()*3).toFixed(0) % 2 == 0){
				this.fontOUYA.draw('PRESS '+this.GUI.labels.start, this.GUI.centerMiddle.x, this.GUI.centerMiddle.y, ig.Font.ALIGN.CENTER );
			}
		...
		if(this.STATE == 'menu' || this.STATE == 'map'){
			...
			if(this.STATE == 'menu'){
				if(this.menu[this.STATE].position == 0){
					this.fontColor.draw('PLAY AGAINST COMPUTER', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[0], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('PLAY AGAINST COMPUTER', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[0], ig.Font.ALIGN.CENTER );
				}
				if(this.menu[this.STATE].position == 1){
					this.fontColor.draw('LOCAL MULTIPLAYER', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[1], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('LOCAL MULTIPLAYER', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[1], ig.Font.ALIGN.CENTER );
				}
				if(this.menu[this.STATE].position == 2){
					this.fontColor.draw('WATCH GAMEPLAY DEMO', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[2], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('WATCH GAMEPLAY DEMO', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[2], ig.Font.ALIGN.CENTER );
				}
		...
		if(this.STATE == 'tutorial'){
			this.fontColor.draw('NEXT PAGE', this.GUI.rightBottom.x , this.GUI.rightBottom.y-8, ig.Font.ALIGN.RIGHT );

			newX = this.GUI.centerMiddle.x;
			newY = this.GUI.centerMiddle.y;

			// TITLE
			this.fontWhite.draw('BIT WARS MANUAL', this.GUI.centerTop.x, this.GUI.centerTop.y, ig.Font.ALIGN.CENTER );

			// SUB TITLE
			if(this.menu[this.STATE].page === 1){
				tutMessage1 = 'Bit Wars is a turn based strategy game for two players.\n'+
					'Each turn player can move the terrain row up or down.\n'+
					'Player do not move units. They move in enemy direction.\n';

				this.tutorialGoalImage.draw(newX-33,newY-24);

				tutMessage2 = 'Goal of the game is to put flags on every spot in enemy line.\n'+
					'Simply move any unit to that spot to put flag in it.';
			}
			if(this.menu[this.STATE].page === 2){
				tutMessage1 = 'You have 3 unit types: Soldier, Tank and Helicopter.\n'+
					'Soldiers are cheapest and Helicopters most expensive.\n'+
					'So spend your money wisely and adequate\n to the battlefield situation.';

				this.tutorialUnitsImage.draw(newX-39,newY-8);

				tutMessage2 = 'Tank is better than Soldier. Helicopter is better than Tank.\n'+
					'Helicopter has one disadvantage: can not capture enemy bases.';
			}
			if(this.menu[this.STATE].page === 3){
				tutMessage1 = 'You control the map by shifting columns.\n'+
					'Each player have '+this.teams[0].moves+' moves on start and each turn gains +1.\n'+
					'Maps are proceduraly generated with means each time different.\n';

					this.tutorialMovesImage.draw(newX-36,newY-32);

				tutMessage2 = 'Maps are made of three types of terrain:\n'+
					'- flat land (grass) - forest (tree) - mountains (rock)\n';
			}
			if(this.menu[this.STATE].page === 4){
				tutMessage1 = 'Each turn units move in the enemy directory if the line is clear.\n'+
						'If there is something other than flat land this rules aplies:\n'+
						'\n';

					this.tutorialTilesImage.draw(newX-73,newY-34);

				tutMessage2 = 'Soldiers can\'t move on mountans, tanks on forests and mountans.\n'+
					'Helicopters fly over any type of terrain.';
			}
			if(this.menu[this.STATE].page === 5){
				tutMessage1 = 'Currency in game is BitCoins (BTC). Players starts with '+this.teams[0].bitcoins+' BTC.\n'+
				'Use them to buy new units or wait, collect and buy better one.\n'+
				'New units arrives on first line of the map on the random empty spot.\n';

				this.tutorialBasesImage.draw(newX-110,newY-12);
				this.tutorialCpusImage.draw(newX+10,newY-12);

				tutMessage2 = 'After each turn player earns +'+this.settings.btcPerTurn+' BTC and +'+this.settings.btcFromBase+' BTC for each\n'+
					'captured base on map. Every five-turn CPU is droped on.\n'+
					'random clear tile. Move any unit on it to gain imediatly +'+this.settings.btcFromCpu+' BTC.\n'+
					'Remember that only ground unit can captur enemy or empty base.';
			}
			if(this.menu[this.STATE].page === 6){
				tutMessage1 = 'Game is easy to play and hard to master.\n'+
						'Practice against computer then challange a  friend for maximum fun.\n';

				this.heroesImage.drawTile(this.GUI.centerMiddle.x-72, this.GUI.centerMiddle.y+20,1,30,28);
				this.heroesImage.drawTile(this.GUI.centerMiddle.x+42, this.GUI.centerMiddle.y+20,0,30,28);

				tutMessage2 = 'Have fun!!!';
			}
			this.fontWhite.draw(tutMessage1, this.GUI.centerTop.x, this.GUI.centerTop.y+24, ig.Font.ALIGN.CENTER );
			if(tutMessage2) this.fontWhite.draw(tutMessage2, this.GUI.centerBottom.x, this.GUI.centerBottom.y-48, ig.Font.ALIGN.CENTER );

			// PAGE
			this.fontWhite.draw('PAGE '+this.menu[this.STATE].page+' / '+this.menu[this.STATE].pages, this.GUI.centerBottom.x, this.GUI.centerBottom.y-8, ig.Font.ALIGN.CENTER );
		...
		if(this.STATE == 'game' || this.STATE == 'move' || this.STATE == 'game_menu' || this.STATE == 'game_over'){
			...
			if(this.STATE == 'game'){
				...
				this.buyButtonsSheet.drawTile(newX-17,newY,this.team-1,32,16);
				this.fontOUYA.draw(this.GUI.labels.next, newX-14, newY+9, ig.Font.ALIGN.CENTER );
				this.fontWhite.draw('DONE', newX-1, newY+3, ig.Font.ALIGN.CENTER );



				this.fontWhite.draw(this.GUI.labels.game_menu, this.GUI.centerBottom.x, this.GUI.centerBottom.y-8, ig.Font.ALIGN.CENTER );

				this.fontRed.draw('PAUSE > MENU', this.GUI.centerBottom.x, this.GUI.centerBottom.y, ig.Font.ALIGN.CENTER );
			...
			if(this.STATE == 'game_over'){
				...
				if(this.menu[this.STATE].position == 1){
					this.fontColor.draw('PLAY NEXT MAP', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[1], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('PLAY NEXT MAP', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[1], ig.Font.ALIGN.CENTER );
				}

				if(this.menu[this.STATE].position == 2){
					this.fontColor.draw('BACK TO MENU', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[2], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('BACK TO MENU', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[2], ig.Font.ALIGN.CENTER );
				}
			}
			...
			if(this.STATE == 'game_menu'){
				...
				if(this.menu[this.STATE].position == 1){
					this.fontColor.draw('SURRENDER > MENU', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[1], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('SURRENDER > MENU', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[1], ig.Font.ALIGN.CENTER );
				}
				...
				if(this.menu[this.STATE].position == 3){
					this.fontColor.draw('GENERATE NEW MAP', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[3], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('GENERATE NEW MAP', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[3], ig.Font.ALIGN.CENTER );
				}

				if(this.menu[this.STATE].position == 4){
					if(this.sound){
						this.fontColor.draw('SOUND: ON', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[4], ig.Font.ALIGN.CENTER );
					}else{
						this.fontColor.draw('SOUND: OFF', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[4], ig.Font.ALIGN.CENTER );
					}
				}else{
					if(this.sound){
						this.fontWhite.draw('SOUND: ON', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[4], ig.Font.ALIGN.CENTER );
					}else{
						this.fontWhite.draw('SOUND: OFF', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[4], ig.Font.ALIGN.CENTER );
					}
				}
