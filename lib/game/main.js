...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		this.GUI.leftTop = {x:16,y:16};
		this.GUI.centerTop = {x:ig.system.width*0.5,y:16};
		this.GUI.rightTop = {x:ig.system.width-16,y:16};
	...
	loadMap: function(){	
		...
		ig.game.spawnEntity( EntityFlag_red, this.map.pointZero.x, this.map.pointZero.y + (((Math.random()*8)<<0)*16) );
		ig.game.spawnEntity( EntityFlag_red, this.map.pointZero.x, this.map.pointZero.y + (((Math.random()*8)<<0)*16) );
		ig.game.spawnEntity( EntityFlag_red, this.map.pointZero.x, this.map.pointZero.y + (((Math.random()*8)<<0)*16) );
		ig.game.spawnEntity( EntityFlag_blue, this.map.pointZero.x + (this.map.blueprint.width*15), this.map.pointZero.y + (((Math.random()*8)<<0)*16) );
		ig.game.spawnEntity( EntityFlag_blue, this.map.pointZero.x + (this.map.blueprint.width*15), this.map.pointZero.y + (((Math.random()*8)<<0)*16) );
	},
	...
	spawnArmy: function(){
		...
		newX = this.map.pointZero.x + (16+(Math.random()*(this.map.blueprint.width-3)*16)<<0);
		newY = this.map.pointZero.y + ((Math.random()*(this.map.blueprint.height-1)*16)<<0);

		if(Math.random()*10 > 5){
			ig.game.spawnEntity( EntitySoldier_blue, newX, newY );
		}else{
			ig.game.spawnEntity( EntitySoldier_red, newX, newY );
		}
	},
	...
	draw: function() {		
		...
		if(this.STATE == 'game'){			
			...
			this.fontWhite.draw('TURN 2 - SUMMER', this.GUI.centerTop.x, this.GUI.centerTop.y, ig.Font.ALIGN.CENTER );		
		}
