...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		this.GUI.buttons.blue = {x:16,y:ig.system.height*0.5+3};
		this.GUI.buttons.red = {x:ig.system.width-16,y:ig.system.height*0.5+3};

		this.menu['menu'].cursor.push(this.GUI.centerMiddle.y-42);
		this.menu['menu'].cursor.push(this.GUI.centerMiddle.y-30);
		this.menu['menu'].cursor.push(this.GUI.centerMiddle.y-18);
...
*/
	...
	draw: function() {		
		...
		if(this.STATE == 'intro'){
			...
			this.fontWhite.draw('Version '+this.version, this.GUI.centerMiddle.x, this.GUI.centerMiddle.y, ig.Font.ALIGN.CENTER );

			if( (this.timer.delta()*3).toFixed(0) % 2 == 0){
				this.fontWhite.draw('PRESS '+this.GUI.labels.start, this.GUI.centerMiddle.x, this.GUI.centerMiddle.y+24, ig.Font.ALIGN.CENTER );
			}
		...
		if(this.STATE == 'game' || this.STATE == 'move' || this.STATE == 'game_menu' || this.STATE == 'game_over'){			
			...
			if(this.STATE == 'game'){
				...
				this.cpuSheet.drawTile(newX, newY-54,0,16,16);
				this.fontWhite.draw(this.teams[this.team-1].bitcoins.toFixed(1), newX+18, newY-48, ig.Font.ALIGN.LEFT );
				
				this.shopSheet.drawTile(newX-4,newY-35,this.team-1,40,70);

				this.fontWhite.draw('SHOP', newX+16, newY-34, ig.Font.ALIGN.CENTER );

				this.buyButtonsSheet.drawTile(newX,newY-22,this.team-1,32,16);
				this.buyButtonsSheet.drawTile(newX,newY-4,this.team-1,32,16);
				this.buyButtonsSheet.drawTile(newX,newY+14,this.team-1,32,16);			

				this.unitsSheet.drawTile(newX+2, newY-3-22, this.team === 1 ? 0 : 3, 16, 16);
				this.unitsSheet.drawTile(newX+2, newY-3-4, this.team === 1 ? 6 : 9, 16, 16);
				this.unitsSheet.drawTile(newX+2, newY-3+14, this.team === 1 ? 12 : 16, 16, 16);

				this.fontWhite.draw('['+ this.GUI.labels._1 +']', newX+2, newY+9-22, ig.Font.ALIGN.CENTER );		
				this.fontWhite.draw('['+ this.GUI.labels._2 +']', newX+2, newY+9-4, ig.Font.ALIGN.CENTER );		
				this.fontWhite.draw('['+ this.GUI.labels._3 +']', newX+2, newY+9+14, ig.Font.ALIGN.CENTER );


				if(this.settings.prices.soldier <= this.teams[this.team-1].bitcoins){
					this.fontWhite.draw(this.settings.prices.soldier, newX+24, newY+2-22, ig.Font.ALIGN.CENTER );
				}else{
					this.fontColor.draw(this.settings.prices.soldier, newX+24, newY+2-22, ig.Font.ALIGN.CENTER );
				}

				if(this.settings.prices.tank <= this.teams[this.team-1].bitcoins){	
					this.fontWhite.draw(this.settings.prices.tank, newX+24, newY+2-4, ig.Font.ALIGN.CENTER );	
				}else{
					this.fontColor.draw(this.settings.prices.tank, newX+24, newY+2-4, ig.Font.ALIGN.CENTER );	
				}

				if(this.settings.prices.helicopter <= this.teams[this.team-1].bitcoins){	
					this.fontWhite.draw(this.settings.prices.helicopter, newX+24, newY+2+14, ig.Font.ALIGN.CENTER );
				}else{
					this.fontColor.draw(this.settings.prices.helicopter, newX+24, newY+2+14, ig.Font.ALIGN.CENTER );
				}
			...
			if(this.STATE == 'game_menu'){
				...
				if(this.menu[this.STATE].position == 2){
					this.fontColor.draw('RESTART', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[2], ig.Font.ALIGN.CENTER );
				}else{
					this.fontWhite.draw('RESTART', this.GUI.centerMiddle.x, this.menu[this.STATE].cursor[2], ig.Font.ALIGN.CENTER );
				}
