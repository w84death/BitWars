...
MyGame = ig.Game.extend({
	...
	buyUnit: function(params){
		...
		if(params.flag){
			x = params.blue ? this.map.blueprint.width-1 : 0;	
			y = params.pos;
		}else{
			x = params.blue ? 0 : this.map.blueprint.width -1;
			y = (Math.random()*(this.map.blueprint.height))<<0;	
		}		
			...
			for (var y = 0; y < this.map.blueprint.height; y++) {
				if(this.map.units[x][y]){
					...
					if(unit.turn == this.turn){
						...
						//console.log('x:'+x, 'target:'+unit.target)	

						if(unit.target > -1){
							if(unit.blue && this.map.pointZero.x + (unit.target * 16) > unit.pos.x){
								unit.vel.x = unit.speed;
								unitsMoving = true;
							}else
							if(unit.red && this.map.pointZero.x + (unit.target * 16) < unit.pos.x){
								unit.vel.x = -unit.speed;
								unitsMoving = true;							
							}else{						
								if((unit.red && unit.target < 1) || (unit.blue && unit.target > (this.map.blueprint.width-2))){									
