...
ig.AI = ig.Class.extend({
	...
	analizeTerrain: function(params){
		...
		if(params && params.calculateMoves){
			console.log('-[ '+ ig.game.turn + ']- ------- AI calculate moves ------');
		}
				...
				if(unit && unit.team === ig.game.team){
					...
					if(!unit.flag){
						...
						if(params && params.calculateMoves){
							...
							console.log(x,y,'search terrain', ig.game.map.terrain[x+dir][y].type, unit.terrain)
							if(ig.game.map.terrain[x+dir][y].type >= unit.terrain && ig.game.map.terrain[x+dir][y].type < 3 ){							
								console.log(x,y,'move terrain')
								...
								console.log(x,y,'search for units')
								// UNITS
								...
								if(enemy && enemy.team !== unit.team){
									console.log(x,y,'found enemy')
									if( enemy.power > unit.power || enemy.flag ){
										console.log(x,y,'enemy power: ' +enemy.power, 'my power: ' + unit.power, 'retreat')
										if( ((Math.random()*10)<<0) <= 5){
											up = false;
											down = true;
										}else{
									...
									if(unit.power > 1 && enemy.power <= unit.power){
										// go for it!
										console.log(x,y,'enemy power: ' +enemy.power, 'my power: ' + unit.power, 'attack')
									}else{									
										// search for cross tiles
										console.log(x,y,'earch for cross tiles')
										// up tile
										if(y > 0){
