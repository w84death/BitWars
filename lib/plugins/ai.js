...
ig.AI = ig.Class.extend({
	...
	makeToDo: function(){
		var dir, enemy, key, newX, newY, q = 1, r = (Math.random()*10)<<0;

		// BUYING
		if(ig.game.turn < 2 && this.stats.enemyUnits.tank === 0){
		...
		if(ig.game.turn < 3 && this.stats.enemyUnits.tank > 0){
		...
		if(this.stats.enemyUnits.all >= this.stats.ownUnits.all || r < 5){
			if(ig.game.teams[ig.game.team-1].bitcoins >= ig.game.settings.prices.helicopter && ( (this.stats.enemyUnits.helicopter > this.stats.ownUnits.helicopter && ig.game.turn > 5 ) || r < 3 )) {

				q = (ig.game.teams[ig.game.team-1].bitcoins / ig.game.settings.prices.helicopter) << 0;
				this.toDo.buy.push({
					unit: 'helicopter',
					quanity: q
				});
			...
			if( (this.stats.enemyUnits.tank > this.stats.ownUnits.tank && ig.game.turn > 1 ) || ( r < 7 && ig.game.teams[ig.game.team-1].bitcoins >= ig.game.settings.prices.tank && this.stats.ownUnits.tank < 5)){
				q = (ig.game.teams[ig.game.team-1].bitcoins / ig.game.settings.prices.tank) << 0;
				this.toDo.buy.push({
					unit: 'tank',
					quanity: q
				});
			...
			if((this.stats.enemyUnits.soldier > this.stats.ownUnits.soldier && r < 6 && this.stats.ownUnits.soldier < 8 ) || this.stats.ownUnits.soldier < 3){
				q = (ig.game.teams[ig.game.team-1].bitcoins / ig.game.settings.prices.soldier) << 0;
				this.toDo.buy.push({
					unit: 'soldier',
					quanity: q
				});
			...
			if(ig.game.map.units[x][y]){
					...
					if(unit && unit.team === ig.game.team && !unit.flag && !unit.base){
						...
						for (var i = 0; i < 3; i++) {
							...
							if(that.heatMap[newX][newY] > top.front){
								top.front = that.heatMap[newX][newY];
								heatMapList.front = {x:newX, y:newY};
							}
							if(that.heatMap[newX][newY] > top.back){
								top.back = that.heatMap[newX][newY]
								heatMapList.back = {x:newX, y:newY};
							}
						}

						if(top.front > top.back && that.clearRows[x+dir]){
							if(y - heatMapList.front.y > 1){
								up = false;
								down = true;
							}else
							if(y - heatMapList.front.y === 1){
		...
		// the blue (left) team shoud first move the units in front
		if(ig.game.team === 1){
			// for x go from right to left
			for (var x = ig.game.map.blueprint.width-1; x > 0; x--) {
				for (var y = 0; y < ig.game.map.blueprint.height; y++) {
					calculate(x,y);
				}
			}
		}else{
			// the red (right) should check from left to right
			for (var x = 0; x < ig.game.map.blueprint.width; x++) {
				for (var y = 0; y < ig.game.map.blueprint.height; y++) {
					calculate(x,y);
				}
	...
	runToDo: function(){
		...
		if(this.status.buying && this.timer.delta() > -1){
