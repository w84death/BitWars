...
ig.ProceduralMap = ig.Class.extend({
	...
	generate: function(params){    	    	
		...
		for (var x = 1; x < this.blueprint.width-1; x++) {
			for (var y = 0; y < this.blueprint.height; y++) {
				freeInRow = 0;						
				if(this.terrain[x][y] && this.terrain[x][y].type === 0){
					freeInRow += 1;
				}				
			}
			if( freeInRow === 0 ){
				randomRow = (Math.random()*this.blueprint.height)<<0;
				this.terrain[x][randomRow].type = 0;
			}
		}
