...
// lib/plugins/ai.js
ig.baked=true;ig.module('plugins.ai').requires('impact.game').defines(function(){ig.AI=ig.Class.extend({team:1,toDo:{buy:[],move:[]},timer:new ig.Timer(),turnTime:1,status:{init:false,thinking:false,buying:false,moveing:false},stats:{ownUnits:{soldier:0,tank:0,helicopter:0,all:0},enemyUnits:{soldier:0,tank:0,helicopter:0,all:0},ownBases:0,enemyBases:0},init:function(){console.log('AI initialized.');},analizeTerrain:function(){var unit,enemy,dir;this.stats.ownUnits.soldier=0;this.stats.ownUnits.tank=0;this.stats.ownUnits.helicopter=0;this.stats.ownUnits.all=0;this.stats.enemyUnits.soldier=0;this.stats.enemyUnits.tank=0;this.stats.enemyUnits.helicopter=0;this.stats.enemyUnits.all=0;dir=(ig.game.team===1?1:-1);for(var x=0;x<ig.game.map.blueprint.width;x++){for(var y=0;y<ig.game.map.blueprint.height;y++){unit=ig.game.getEntityByName(ig.game.map.units[x][y]);if(unit&&unit.team===ig.game.team){if(unit.base){this.stats.ownBases+=1;}else
...
enemy=ig.game.getEntityByName(ig.game.map.units[x+dir][y]);if(enemy&&(enemy.power>=unit.power||enemy.flag)){this.toDo.move.push({row:x,quanity:1,down:true});this.status.moveing=true;}
...
if(enemy&&enemy.base){this.toDo.move.push({row:x+dir,down:true,quanity:1});this.status.moveing=true;}
enemy=false;if(y<ig.game.map.blueprint.height-1){enemy=ig.game.getEntityByName(ig.game.map.units[x+dir][y+1]);}else{enemy=ig.game.getEntityByName(ig.game.map.units[x+dir][0]);}
if(enemy&&enemy.base){this.toDo.move.push({row:x+dir,top:true,quanity:1});this.status.moveing=true;}
enemy=false;if(ig.game.map.terrain[x+dir][y].type>=unit.terrain&&ig.game.map.terrain[x+dir][y].type<3){this.toDo.move.push({row:x+dir,quanity:1,top:true});this.status.moveing=true;}}}
if(unit&&unit.team!==ig.game.team){if(ig.game.map.units[x][y].base){this.stats.enemyBases+=1;}else
if(!unit.flag){if(unit.soldier)this.stats.enemyUnits.soldier+=1;if(unit.tank)this.stats.enemyUnits.tank+=1;if(unit.helicopter)this.stats.enemyUnits.helicopter+=1;this.stats.enemyUnits.all+=1}}}};},makeToDo:function(){this.analizeTerrain();console.log(ig.game.team,this.stats.ownUnits.soldier);var r=(Math.random()*10)<<0;if(ig.game.turn<2){this.toDo.buy.push({unit:'soldier',quanity:3});this.toDo.buy.push({unit:'tank',quanity:1});}else
...
this.status.buying=true;},runToDo:function(){if(this.status.buying&&this.timer.delta()>-0.8){for(var i=0;i<this.toDo.buy.length;i++){for(var q=0;q<this.toDo.buy[i].quanity;q++){ig.game.buyUnit({unit:this.toDo.buy[i].unit,team:ig.game.team});};};this.status.buying=false;}
if(this.status.moveing&&this.timer.delta()>-0.5){if(this.toDo.move.length>0){ig.game.moveRuler({setRow:this.toDo.move[0].row,});for(var y=0;y<this.toDo.move[0].quanity;y++){if(this.toDo.move[0].top){ig.game.moveRuler({up:true});}else{ig.game.moveRuler({down:true});}};}}
if(this.status.moveing&&this.timer.delta()>-0.3){if(this.toDo.move.length>1){for(var i=1;i<this.toDo.move.length;i++){if(this.toDo.move[0].top){ig.game.moveRuler({setRow:this.toDo.move[i].row,up:true});}else
if(this.toDo.move[0].down){ig.game.moveRuler({setRow:this.toDo.move[i].row,down:true});}else{if(Math.random()>0.5){ig.game.moveRuler({setRow:this.toDo.move[i].row,up:true});}else{ig.game.moveRuler({setRow:this.toDo.move[i].row,down:true});}}};}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','plugins.gamepad','plugins.procedural-map','plugins.ai','game.entities.terrain','game.entities.grass','game.entities.tree','game.entities.rock','game.entities.grass_left','game.entities.grass_right','game.entities.base','game.entities.unit','game.entities.soldier_blue','game.entities.soldier_red','game.entities.tank_blue','game.entities.tank_red','game.entities.helicopter_blue','game.entities.helicopter_red','game.entities.flag','game.entities.flag_red','game.entities.flag_blue').defines(function(){MyGame=ig.Game.extend({version:'alpha 0.04',clearColor:'#b2dcef',fontWhite:new ig.Font('media/font/white.png'),fontColor:new ig.Font('media/font/colors.png'),fontRed:new ig.Font('media/font/red.png'),fontBlue:new ig.Font('media/font/blue.png'),logoImage:new ig.Image('media/gfx/logo.png'),heroesImage:new ig.Image('media/gfx/heroes.png'),mapFrameSheet:new ig.Image('media/gfx/frame.png'),cornerSheet:new ig.Image('media/gfx/corner.png'),buyButtonsSheet:new ig.Image('media/gfx/buy_buttons.png'),unitsSheet:new ig.Image('media/units/units.png'),rulerSheet:new ig.Image('media/gfx/ruler.png'),debug:false,STATE:'intro',turn:0,team:1,teams:[{ai:true,score:0,bitcoins:8,moves:4,},{ai:true,score:0,bitcoins:8,moves:6,}],timer:new ig.Timer(),GUI:{leftTop:null,middleTop:null,rightTop:null,centerBottom:null,map:null,buttons:{blue:null,red:null},labels:{next:'spacebar',_1:'1',_2:'2',_3:'3',}},settings:{prices:{soldier:1,tank:3,helicopter:6},btcPerTurn:0.2,btcFromBase:0.5,},map:{},units:{},ruler:[2,6],menu:{cursor:[],position:0},ai:true,init:function(){console.log('\n\nWelcom to the Bit Wars ['+this.version+']\n\nSource code: https://github.com/w84death/BitWars/\n\n--- \n(c)2014 Krzysztof Jankowski\n');this.GUI.leftTop={x:16,y:8};this.GUI.centerTop={x:ig.system.width*0.5,y:8};this.GUI.rightTop={x:ig.system.width-16,y:8};this.GUI.centerBottom={x:ig.system.width*0.5,y:ig.system.height-8};this.GUI.centerMiddle={x:ig.system.width*0.5,y:ig.system.height*0.5}
this.GUI.map={x:ig.system.width*0.5,y:ig.system.height*0.5};this.GUI.buttons.blue={x:16,y:ig.system.height*0.5};this.GUI.buttons.red={x:ig.system.width-16,y:ig.system.height*0.5};ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY._1,'buy_1');ig.input.bind(ig.KEY._2,'buy_2');ig.input.bind(ig.KEY._3,'buy_3');ig.input.bind(ig.KEY.SPACE,'next_turn');ig.input.bind(ig.KEY.ENTER,'action');Gamepad.mappings.one=[['dpadUp','up'],['dpadDown','down'],['dpadLeft','left'],['dpadRight','right'],['leftStickX','left','right'],['leftStickY','up','down'],['start','action'],['faceButton0','buy_1'],['faceButton1','buy_2'],['faceButton2','buy_3'],['faceButton3','next_turn']];this.fontWhite.letterSpacing=-1;this.fontWhite.lineSpacing=-2;this.fontColor.letterSpacing=-1;this.fontColor.lineSpacing=-2;this.fontRed.letterSpacing=-1;this.fontRed.lineSpacing=-2;this.fontBlue.letterSpacing=-1;this.fontBlue.lineSpacing=-2;},proceduralMap:function(){var newProceduralMap=new ig.ProceduralMap();newProceduralMap.generate({prefs:{width:12,height:6,season:'summer'},dna:{trees:0.2,rocks:0.1,bases:4}})
this.map=newProceduralMap.get();this.map.pointZero={x:this.GUI.map.x-(this.map.blueprint.width*8),y:this.GUI.map.y-(this.map.blueprint.height*8)+8}},stateChange:function(state){if(this.STATE=='intro'&&state=='menu'){this.menu.cursor.push(this.GUI.centerMiddle.y-24);this.menu.cursor.push(this.GUI.centerMiddle.y-12);this.menu.cursor.push(this.GUI.centerMiddle.y);this.STATE='menu';}
if(this.STATE=='menu'&&state=='newGame'){this.STATE='game';if(this.menu.position===0){this.teams[0].ai=false;this.teams[1].ai=true;}
if(this.menu.position===1){this.teams[0].ai=false;this.teams[1].ai=false;}
if(this.menu.position===2){this.teams[0].ai=true;this.teams[1].ai=true;}
this.ai=new ig.AI();this.proceduralMap();this.loadMap();}},loadMap:function(){var newX=0,newY=0,name='';for(var x=0;x<this.map.blueprint.width;x++){for(var y=0;y<this.map.blueprint.height;y++){newX=(x*16)+this.map.pointZero.x;newY=(y*16)+this.map.pointZero.y;name='terrain_'+Math.random().toString();this.map.terrain[x][y].name=name;if(this.map.terrain[x][y].type===0){ig.game.spawnEntity(EntityGrass,newX,newY,{name:name});}
...
if(params.down){this.moveRow({row:this.ruler[this.team-1],down:true});}},moveCursor:function(params){if(params.menu){if(params.up){this.menu.position-=1;if(this.menu.position<0){this.menu.position=this.menu.cursor.length-1;}}
if(params.down){this.menu.position+=1;if(this.menu.position>this.menu.cursor.length-1){this.menu.position=0;}}}},update:function(){Gamepad.handleInput();this.parent();if(this.STATE=='intro'){if(ig.input.released('action')||ig.input.state('touch')){this.stateChange('menu');return;}}
if(this.STATE=='menu'){if(ig.input.released('up')){this.moveCursor({up:true,menu:true});}
if(ig.input.released('down')){this.moveCursor({down:true,menu:true});}
if(ig.input.released('action')||ig.input.state('touch')){this.stateChange('newGame');return;}}
if(this.STATE=='game'){if(this.teams[this.team-1].ai){this.ai.think();if(this.ai.timer.delta()>0){this.STATE='move';}}else{if(ig.input.released('buy_1')){this.buyUnit({unit:'soldier',team:this.team});}
...
if(this.STATE=='menu'){this.logoImage.draw(this.GUI.centerTop.x-24,this.GUI.centerTop.y+6);if(this.menu.position==0){this.fontColor.draw('HUMAN VS AI',this.GUI.centerMiddle.x,this.menu.cursor[0],ig.Font.ALIGN.CENTER);}else{this.fontWhite.draw('HUMAN VS AI',this.GUI.centerMiddle.x,this.menu.cursor[0],ig.Font.ALIGN.CENTER);}
if(this.menu.position==1){this.fontColor.draw('HUMAN VS HUMAN',this.GUI.centerMiddle.x,this.menu.cursor[1],ig.Font.ALIGN.CENTER);}else{this.fontWhite.draw('HUMAN VS HUMAN',this.GUI.centerMiddle.x,this.menu.cursor[1],ig.Font.ALIGN.CENTER);}
if(this.menu.position==2){this.fontColor.draw('AI VS AI',this.GUI.centerMiddle.x,this.menu.cursor[2],ig.Font.ALIGN.CENTER);}else{this.fontWhite.draw('AI VS AI',this.GUI.centerMiddle.x,this.menu.cursor[2],ig.Font.ALIGN.CENTER);}
this.heroesImage.drawTile(this.GUI.centerTop.x-72,this.menu.cursor[this.menu.position]-12,1,30,28);this.heroesImage.drawTile(this.GUI.centerTop.x+42,this.menu.cursor[this.menu.position]-12,0,30,28);this.fontColor.draw('QUICK TUTORIAL\n'+'Move terrain (without enemy units) up or down.\n'+'Manage to cross enemy lines with your units.',this.GUI.centerBottom.x,this.GUI.centerBottom.y-38,ig.Font.ALIGN.CENTER);}
if(this.STATE=='game'||this.STATE=='move'||this.STATE=='terrain'||this.STATE=='game_over'){this.renderMapFrame();if(this.STATE=='game'){var newX,newY;newX=this.team===1?this.GUI.buttons.blue.x:this.GUI.buttons.red.x-32,newY=this.team===1?this.GUI.buttons.blue.y:this.GUI.buttons.red.y;this.fontWhite.draw('BUY UNITS:\nYOU HAVE: '+this.teams[this.team-1].bitcoins.toFixed(1),newX+14,newY-48,ig.Font.ALIGN.CENTER);this.buyButtonsSheet.drawTile(newX,newY-24,this.team-1,32,16);this.buyButtonsSheet.drawTile(newX,newY,this.team-1,32,16);this.buyButtonsSheet.drawTile(newX,newY+24,this.team-1,32,16);this.unitsSheet.drawTile(newX+2,newY-3-24,this.team===1?0:3,16,16);this.unitsSheet.drawTile(newX+2,newY-3,this.team===1?6:9,16,16);this.unitsSheet.drawTile(newX+2,newY-3+24,this.team===1?12:16,16,16);this.fontWhite.draw('['+this.GUI.labels._1+']',newX+2,newY+9-24,ig.Font.ALIGN.CENTER);this.fontWhite.draw('['+this.GUI.labels._2+']',newX+2,newY+9,ig.Font.ALIGN.CENTER);this.fontWhite.draw('['+this.GUI.labels._3+']',newX+2,newY+9+24,ig.Font.ALIGN.CENTER);if(this.settings.prices.soldier<=this.teams[this.team-1].bitcoins){this.fontWhite.draw('1',newX+24,newY+2-24,ig.Font.ALIGN.CENTER);}else{this.fontColor.draw('1',newX+24,newY+2-24,ig.Font.ALIGN.CENTER);}
...
if(this.STATE=='game_over'){this.fontWhite.draw('TEAM '+(this.teams[0].score>this.teams[1].score?'BLUE':'RED')+' WINS!',this.GUI.centerMiddle.x,this.GUI.centerMiddle.y,ig.Font.ALIGN.CENTER);}
