...
enemy=ig.game.getEntityByName(ig.game.map.units[x+dir][y]);if(enemy&&((ig.game.team===1?enemy.power>=unit.power:enemy.power>unit.power)||enemy.flag)){this.toDo.move.push({row:x,quanity:1,down:true});this.status.moveing=true;}
...
this.STATE='map';}
if(this.STATE=='map'&&state=='game'){var params={};if(this.menu['map'].position===0){params.width=10;params.height=4;}
if(this.menu['map'].position===1){params.width=12;params.height=6;}
if(this.menu['map'].position===2){params.width=14;params.height=7;}
...
if(this.STATE=='game'&&state=='game_over'){this.STATE='game_over';}
if((this.STATE=='game_over'&&state=='menu')||(this.STATE=='game_menu'&&state=='menu')){this.ai=null;this.unloadMap();this.STATE='menu';}},unloadMap:function(){for(var i=0;i<this.entities.length;i++){this.entities[i].kill();}
this.turn=0;this.team=1;this.teams[0].score=0;this.teams[0].bitcoins=8;this.teams[0].moves=4;this.teams[1].score=0;this.teams[1].bitcoins=8;this.teams[1].moves=4;this.timer.set(0);},loadMap:function(){var newX=0,newY=0,name='';for(var x=0;x<this.map.blueprint.width;x++){for(var y=0;y<this.map.blueprint.height;y++){newX=(x*16)+this.map.pointZero.x;newY=(y*16)+this.map.pointZero.y;name='terrain_'+Math.random().toString();this.map.terrain[x][y].name=name;if(this.map.terrain[x][y].type===0){ig.game.spawnEntity(EntityGrass,newX,newY,{name:name});}
...
if(params.down){this.moveRow({row:this.ruler[this.team-1],down:true});}},moveCursor:function(params){if(params.up){this.menu[this.STATE].position-=1;if(this.menu[this.STATE].position<0){this.menu[this.STATE].position=this.menu[this.STATE].cursor.length-1;}}
...
if(this.STATE=='menu'||this.STATE=='map'){this.logoImage.draw(this.GUI.centerTop.x-24,this.GUI.centerTop.y+6);if(this.STATE=='map'){if(this.menu[this.STATE].position==0){this.fontColor.draw('SMALL 10x4 MAP',this.GUI.centerMiddle.x,this.menu[this.STATE].cursor[0],ig.Font.ALIGN.CENTER);}else{this.fontWhite.draw('SMALL 10x4 MAP',this.GUI.centerMiddle.x,this.menu[this.STATE].cursor[0],ig.Font.ALIGN.CENTER);}
if(this.menu[this.STATE].position==1){this.fontColor.draw('NORMAL 12x6 MAP',this.GUI.centerMiddle.x,this.menu[this.STATE].cursor[1],ig.Font.ALIGN.CENTER);}else{this.fontWhite.draw('NORMAL 12x6 MAP',this.GUI.centerMiddle.x,this.menu[this.STATE].cursor[1],ig.Font.ALIGN.CENTER);}
if(this.menu[this.STATE].position==2){this.fontColor.draw('BIG 14x7 MAP',this.GUI.centerMiddle.x,this.menu[this.STATE].cursor[2],ig.Font.ALIGN.CENTER);}else{this.fontWhite.draw('BIG 14x7 MAP',this.GUI.centerMiddle.x,this.menu[this.STATE].cursor[2],ig.Font.ALIGN.CENTER);}}
...
this.heroesImage.drawTile(this.GUI.centerTop.x-72,this.menu[this.STATE].cursor[this.menu[this.STATE].position]-12,1,30,28);this.heroesImage.drawTile(this.GUI.centerTop.x+42,this.menu[this.STATE].cursor[this.menu[this.STATE].position]-12,0,30,28);this.fontColor.draw('QUICK TUTORIAL\n'+'Move terrain (without enemy units) up or down.\n'+'Manage to cross enemy lines with your units.',this.GUI.centerBottom.x,this.GUI.centerBottom.y-38,ig.Font.ALIGN.CENTER);}
