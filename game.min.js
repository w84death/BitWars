...
if(!unit.flag&&!unit.cpu){if(unit.soldier)this.stats.enemyUnits.soldier+=1;if(unit.tank)this.stats.enemyUnits.tank+=1;if(unit.helicopter)this.stats.enemyUnits.helicopter+=1;this.stats.enemyUnits.all+=1}}}}},makeToDo:function(){var dir,enemy,key,newX,newY,r=(Math.random()*10)<<0;if(ig.game.turn<2&&this.stats.enemyUnits.helicopter===0){this.toDo.buy.push({unit:'soldier',quanity:3});this.toDo.buy.push({unit:'tank',quanity:1});}else
...
if(unit&&unit.team===ig.game.team){this.clearRows[x]==false;}}}};this.clearRows[0]==false;this.clearRows[this.clearRows.length-1]==false;var heatMapList={front:{},back:{}},top={front:-10,back:-10};for(var x=0;x<ig.game.map.blueprint.width;x++){for(var y=0;y<ig.game.map.blueprint.height;y++){if(ig.game.map.units[x][y]){unit=ig.game.getEntityByName(ig.game.map.units[x][y]);if(unit&&unit.team===ig.game.team&&!unit.flag&&!unit.base){for(var i=0;i<3;i++){newX=x+dir;if(i===0)newY=y-1;if(i===1)newY=y;if(i===2)newY=y+1;if(newY<0){newY=ig.game.map.blueprint.height-1;}
if(newY>ig.game.map.blueprint.height-1){newY=0;}
if(this.heatMap[newX][newY]>top.front){top.front=this.heatMap[newX][newY];heatMapList.front={x:newX,y:newY};}
if(this.heatMap[newX][newY]>top.back){top.back=this.heatMap[newX][newY]
heatMapList.back={x:newX,y:newY};}}
if(top.front>top.back&&this.clearRows[x+dir]){if(y-heatMapList.front.y>1){up=false;down=true;}else
...
if(up||down){this.toDo.move.push({row:x+dir,up:up,down:down});}}else
if(this.clearRows[x]){if(y-heatMapList.back.y>1){up=false;down=true;}else
...
if(up||down){this.toDo.move.push({row:x,up:up,down:down});}}}}}}
...
ig.baked=true;ig.module('game.entities.unit').requires('impact.entity','impact.entity-pool').defines(function(){EntityUnit=ig.Entity.extend({type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,animSheet:new ig.AnimationSheet('media/units/units.png',16,16),size:{x:16,y:16},speed:100,init:function(x,y,settings){this.parent(x,y,settings);this.name=settings.name;this.turn=settings.turn;},reset:function(x,y,settings){this.parent(x,y,settings);this.name=settings.name;this.turn=settings.turn;},update:function(){this.parent();if(ig.game.fun){if(((this.pos.y/16)<<0)%2){this.currentAnim.angle-=Math.PI/9*ig.system.tick;}else{this.currentAnim.angle+=Math.PI/9*ig.system.tick;}}},});ig.EntityPool.enableFor(EntityUnit);});
...
if(this.STATE=='tutorial'&&state=='menu'){this.STATE='menu';}
...
this.STATE='map';}
if(this.STATE=='map'&&state=='game'){ig.music.stop();if(this.sound)ig.music.play('game');ig.music.volume=0.4;var params={};if(this.menu['map'].position===0){params.width=9;params.height=4;params.trees=0.15;params.rocks=0.05;params.bases=2;}
...
if((this.STATE=='game_menu'||this.STATE=='game_over')&&state=='restart'){this.ai=null;this.unloadMap();this.STATE='game';this.ai=new ig.AI();this.loadMap();}},unloadMap:function(){for(var i=0;i<this.entities.length;i++){this.entities[i].kill();}
...
if(this.teams[this.team-1].ai){this.ai.unlock();}
...
return;}}
if(this.STATE=='tutorial'){if(ig.input.released('action')||ig.input.released('next_turn')){this.stateChange('menu');}}
...
newX=(this.team===1?this.GUI.shop.blue.x+21:this.GUI.shop.red.x-21);newY=(this.team===1?this.GUI.shop.blue.y+40:this.GUI.shop.red.y+40);this.buyButtonsSheet.drawTile(newX-16,newY,this.team-1,32,16);this.fontWhite.draw('['+this.GUI.labels.next+']',newX,newY+16,ig.Font.ALIGN.CENTER);this.fontWhite.draw('DONE',newX,newY+3,ig.Font.ALIGN.CENTER);this.drawRuler();if(this.team===1){this.fontBlue.draw(this.teams[0].moves,(this.ruler[0]*16)+this.map.pointZero.x,this.map.pointZero.y+(this.map.blueprint.height*16)+8,ig.Font.ALIGN.CENTER);}else{this.fontRed.draw(this.teams[1].moves,(this.ruler[1]*16)+this.map.pointZero.x,this.map.pointZero.y+(this.map.blueprint.height*16)+8,ig.Font.ALIGN.CENTER);}
