...
// lib/plugins/ai.js
ig.baked=true;ig.module('plugins.ai').requires('impact.game').defines(function(){ig.AI=ig.Class.extend({team:1,toDo:{buy:[],move:[]},timer:new ig.Timer(),turnTime:1,status:{init:false,thinking:false,buying:false,moveing:false},stats:{ownUnits:{soldier:0,tank:0,helicopter:0,all:0},enemyUnits:{soldier:0,tank:0,helicopter:0,all:0},ownBases:0,enemyBases:0},log:false,init:function(){if(this.log)console.log('AI initialized. Now will conquer the world!');},analizeTerrain:function(params){var unit,enemy,dir,up,down;this.stats.ownUnits.soldier=0;this.stats.ownUnits.tank=0;this.stats.ownUnits.helicopter=0;this.stats.ownUnits.all=0;this.stats.enemyUnits.soldier=0;this.stats.enemyUnits.tank=0;this.stats.enemyUnits.helicopter=0;this.stats.enemyUnits.all=0;dir=(ig.game.team===1?1:-1);if(params&&params.calculateMoves){if(this.log)console.log('-[ '+ig.game.turn+']- ------- AI calculate moves ------');}
...
this.toDo.move.push({row:x,quanity:1,up:up,down:down});this.status.moveing=true;}else{if(this.log)console.log(x,y,'search for units')
...
if(this.status.moveing&&this.timer.delta()>-0.5){if(this.toDo.move.length>0){ig.game.moveRuler({setRow:this.toDo.move[0].row});for(var y=0;y<this.toDo.move[0].quanity;y++){ig.game.moveRuler({up:this.toDo.move[0].up,down:this.toDo.move[0].down});};}}
