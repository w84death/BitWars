...
if(unit.team===2&&this.map.pointZero.x+(unit.target*16)<unit.pos.x){unit.vel.x=-unit.speed;unitsMoving=true;}else{if((unit.team===2&&unit.target<1)||(unit.team===1&&unit.target>(this.map.blueprint.width-2))){this.buyUnit({team:unit.team,unit:'flag',pos:y});this.map.units[x][y]=false;unit.target=false;unit.kill();}else
if(unit.willCapture&&this.map.terrain[unit.target][y].name){var base=this.getEntityByName(this.map.terrain[unit.target][y].name);base.capture(unit.team);this.map.units[x][y]=false;unit.target=false;unit.kill();}else{if(unit.willAttack&&this.map.units[unit.target][y]){enemy=this.getEntityByName(this.map.units[unit.target][y]);if(enemy.power<unit.power){enemy.kill();this.map.units[unit.target][y]=false;}else
if(enemy.power==unit.power){enemy.kill();this.map.units[unit.target][y]=false;this.map.units[x][y]=false;unit.target=false;unit.willAttack=false;unit.kill();}else{this.map.units[x][y]=false;unit.target=false;unit.willAttack=false;unit.kill();}}
if(unit.target){unit.name=this.map.units[x][y];this.map.units[unit.target][y]=this.map.units[x][y];this.map.units[x][y]=false;unit.target=false;unit.turn=this.turn+1;unit.vel.x=0;unit.willCapture=false;unit.willAttack=false;}}}}}}}};if(!unitsMoving){this.STATE='game';this.nextTurn();for(var x=0;x<this.map.blueprint.width;x++){for(var y=0;y<this.map.blueprint.height;y++){unit=this.getEntityByName(this.map.units[x][y]);if(unit){unit.target=false;unit.turn=this.turn;}}}}},moveRow:function(params){var temp={},tile=null;if(params.up){temp.type=this.map.terrain[params.row][0].type;temp.name=this.map.terrain[params.row][0].name;temp.base=this.map.terrain[params.row][0].base;for(var y=1;y<this.map.blueprint.height;y++){tile=this.getEntityByName(this.map.terrain[params.row][y].name);tile.pos.y-=16;this.map.terrain[params.row][y-1].type=this.map.terrain[params.row][y].type;this.map.terrain[params.row][y-1].name=this.map.terrain[params.row][y].name;this.map.terrain[params.row][y-1].base=this.map.terrain[params.row][y].base;}
this.map.terrain[params.row][this.map.blueprint.height-1].type=temp.type;this.map.terrain[params.row][this.map.blueprint.height-1].name=temp.name;this.map.terrain[params.row][this.map.blueprint.height-1].base=temp.base;tile=this.getEntityByName(this.map.terrain[params.row][this.map.blueprint.height-1].name);tile.pos.y=this.map.pointZero.y+(this.map.blueprint.height-1)*16;temp={};}
if(params.down){temp.type=this.map.terrain[params.row][this.map.blueprint.height-1].type;temp.name=this.map.terrain[params.row][this.map.blueprint.height-1].name;temp.base=this.map.terrain[params.row][this.map.blueprint.height-1].base;for(var y=this.map.blueprint.height-2;y>-1;y--){tile=this.getEntityByName(this.map.terrain[params.row][y].name);tile.pos.y+=16;this.map.terrain[params.row][y+1].type=this.map.terrain[params.row][y].type;this.map.terrain[params.row][y+1].name=this.map.terrain[params.row][y].name;this.map.terrain[params.row][y+1].base=this.map.terrain[params.row][y].base;}
this.map.terrain[params.row][0].type=temp.type;this.map.terrain[params.row][0].name=temp.name;this.map.terrain[params.row][0].base=temp.base;tile=this.getEntityByName(this.map.terrain[params.row][0].name);tile.pos.y=this.map.pointZero.y;temp={};}},moveRuler:function(params){if(params.right){this.ruler[this.team-1]+=1;if(this.ruler[this.team-1]>this.map.blueprint.width-2){this.ruler[this.team-1]=1;}}
...
if(params.down){this.moveRow({row:this.ruler[this.team-1],down:true});}},update:function(){this.parent();if(this.STATE=='intro'){if(ig.input.released('action')||ig.input.state('touch')){this.stateChange('newGame');return;}}
