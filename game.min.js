...
if(top.front>top.back&&this.clearRows[x+dir]){if(heatMapList.front.y<y){up=true;down=false;}else
if(heatMapList.front.y>y){up=false;down=true;}else{up=false;down=false;}
if(up||down){this.toDo.move.push({row:x+dir,up:up,down:down});}}else
if(this.clearRows[x]){if(heatMapList.back.y<y){up=true;down=false;}else
if(heatMapList.back.y>y){up=false;down=true;}else{up=false;down=false;}
...
this.team+=1;if(this.team>2){this.team=1;this.turn+=1;this.giveBTCtoPlayers();this.teams[0].moves+=1;this.teams[1].moves+=1;if(this.turn%5==0){this.spawnCpu();}}
if(this.teams[this.team-1].ai){this.ai.unlock();}},moveUnits:function(){var unit=false,unitsMoving=false,base=null,enemy=null,targetX=false;for(var x=0;x<this.map.blueprint.width;x++){for(var y=0;y<this.map.blueprint.height;y++){if(this.map.units[x][y]){unit=this.getEntityByName(this.map.units[x][y]);if(unit.turn===this.turn&&unit.team===this.team){if(!unit.flag){if(unit.target===false){if(unit.team===1){targetX=x+1;}else{targetX=x-1;}
