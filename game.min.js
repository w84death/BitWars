...
if(unit&&unit.team!==ig.game.team){blackRows[x]=true;if(ig.game.map.units[x][y].base){this.stats.enemyBases+=1;}else
...
this.status.buying=true;dir=(ig.game.team===1?1:-1);for(var x=0;x<ig.game.map.blueprint.width;x++){for(var y=0;y<ig.game.map.blueprint.height;y++){this.clearRows.push(true);if(ig.game.map.units[x][y]){unit=ig.game.getEntityByName(ig.game.map.units[x][y]);if(unit&&unit.team===ig.game.team&&!unit.base&&!unit.flag){for(var i=0;i<3;i++){if(i===0)newY=y-1;if(i===1)newY=y;if(i===2)newY=y+1;if(ig.game.map.units[x+dir][newY]){enemy=ig.game.getEntityByName(ig.game.map.units[x+dir][newY]);if(enemy.flag){key='flag';}else
...
this.heatMap[x+dir][newY]+=this.heatMapKey[key][0];this.heatMap[x][newY]+=this.heatMapKey[key][1];}};for(var i=0;i<3;i++){if(i===0)newY=y-1;if(i===1)newY=y;if(i===2)newY=y+1;if(ig.game.map.terrain[x+dir][newY]){if(ig.game.map.terrain[x+dir][newY].type>=unit.terrain&&ig.game.map.terrain[x+dir][newY].type<3){key='terrain_blocked';}else{key='terrain_clear';}
this.heatMap[x+dir][newY]+=this.heatMapKey[key][0];this.heatMap[x][newY]+=this.heatMapKey[key][1];}};}else
if(unit&&unit.team===ig.game.team){this.clearRows[x]==false;}}}};this.clearRows[0]==false;this.clearRows[this.clearRows.length-1]==false;var heatMapList={front:{},back:{}},top={front:-10,back:-10};for(var x=0;x<ig.game.map.blueprint.width;x++){for(var y=0;y<ig.game.map.blueprint.height;y++){if(ig.game.map.units[x][y]){unit=ig.game.getEntityByName(ig.game.map.units[x][y]);if(unit&&unit.team===ig.game.team&&!unit.flag&&!unit.base){for(var i=0;i<3;i++){newX=x+dir;if(i===0)newY=y-1;if(i===1)newY=y;if(i===2)newY=y+1;if(this.heatMap[newX][newY]>top.front){top.front=this.heatMap[newX][newY];heatMapList.front={x:newX,y:newY};}}
for(var i=0;i<2;i++){newX=x;if(i===0)newY=y-1;if(i===1)newY=y+1;if(this.heatMap[newX][newY]>top.back){top.back=this.heatMap[newX][newY]
...
return;}}
if(this.STATE=='tutorial'){if(ig.input.released('action')){this.stateChange('menu');}}
if(this.STATE=='game'){if(ig.input.released('action')){this.stateChange('game_menu');}else
if(ig.input.released('extras')){this.debug=!this.debug;}else
